...
- `~/.config/agent-box` (`AGENT_CONFIG`): per-tool config/state, wired into the sandbox by the `BIND_DIRS`/`BIND_FILES` tables — each entry is a full `host-source:sandbox-path` pair bound straight onto the tool's expected path (`~/.claude`, `~/.codex`, opencode's XDG config/share/state, `~/.config/{gh,glab-cli,git}` for the git-hosting CLIs + git identity). Because the source is a full path, disposable state goes to the cache dir, e.g. opencode's cache binds from `${AGENT_CACHE}/opencode` while its config binds from `${AGENT_CONFIG}`. `GIT_CONFIG_GLOBAL` is set to the bound `~/.config/git/config` so `git config --global` writes persist (not to the tmpfs `~/.gitconfig`). `~/.claude.json` is a **file bind**; `ensure_sources` seeds missing JSON files with `{}` (opencode rejects a present-but-invalid file).
