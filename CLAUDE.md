...
- `~/.config/agent-box` (`AGENT_CONFIG`): per-tool **config**, wired in by the `BIND_DIRS`/`BIND_FILES` tables — each entry is a full `host-source:sandbox-path` pair bound straight onto the tool's expected path (`~/.claude`, `~/.codex`, opencode's config, `~/.config/{git,gh,glab-cli}` for git + the git-hosting CLIs). Because the source is a full path, each XDG category binds from the matching base: opencode's config from `${AGENT_CONFIG}`, data from `${AGENT_TOOLS}`, state from `${AGENT_STATE}`, cache from `${AGENT_CACHE}` (each `agent-box/opencode` → the real `~/.config`,`~/.local/share`,`~/.local/state`,`~/.cache` path). `~/.claude.json` is a **file bind** (`BIND_FILES`, seeded `{}` if absent — must be valid JSON). **git is the exception:** a *file* bind of `~/.gitconfig` can't be rewritten (git writes via temp+rename → `EBUSY` on the mountpoint), so git binds the `~/.config/git` **dir** with an empty seeded `config` (`SEED_FILES`) and writes there natively — no `GIT_CONFIG_GLOBAL`.
