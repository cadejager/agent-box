...
**Auto-install on first run (`ensure_tools` → `install_tools`).** If `${AGENT_BIN}`/node/`uv` are missing (or `AGTBOX_REINSTALL=1`), install runs **inside a bwrap sandbox** (the toolchain + cache dirs rw, system ro, `$HOME` tmpfs) so the official installers can't touch the host: download the pinned `NODE_VERSION` tarball (arch from `uname -m`) into `${AGENT_TOOLS}/node`, `npm install -g --prefix ${AGENT_TOOLS}` the `NPM_PKGS`, then the `uv` installer (`UV_INSTALL_DIR=${AGENT_TOOLS}/bin`). Idempotent (re-run upgrades). Needs host `curl`/`tar`/`xz`.
