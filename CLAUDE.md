...
- `~/.config/agent-box` (`AGENT_CONFIG`): per-tool config/state, wired into the sandbox by the `BIND_DIRS`/`BIND_FILES` tables — each entry is a full `host-source:sandbox-path` pair bound straight onto the tool's expected path (`~/.claude`, `~/.codex`, opencode's XDG config/share/state). **No symlinks anywhere.** Because the source is a full path, disposable state goes to the cache dir, e.g. opencode's cache binds from `${AGENT_CACHE}/opencode` while its config binds from `${AGENT_CONFIG}`. `~/.claude.json` is a robust **file bind**; `ensure_sources` seeds missing JSON files with `{}` (opencode rejects a present-but-invalid file).
- `~/.cache/agent-box` (`AGENT_CACHE`): npm/pip/uv download caches.

**Env union.** `ENV_FORWARD` (`ANTHROPIC_*`/`OPENAI_API_KEY`/`CODEX_API_KEY`) forwarded when set; `ENV_LITERAL` (`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` + `OPENCODE_*`) always. All emitted as `--setenv NAME VALUE`. Tool-independent — a tool ignores env it doesn't read. (No `TZ` is set — host-local time comes from `/etc/localtime`, visible via the `/etc` bind; verified `date`/`python`/`node` all report the host zone.)
