...
- **Disposable container, persistent config.** Containers run `--rm`. Your host config/login dirs are bind-mounted so sessions persist — but anything installed *globally* inside the container (apt/pip/npm) does **not** survive. Keep project dependencies in the project (a `.venv`, `node_modules`, …); those live in the mounted dir and persist across runs.
