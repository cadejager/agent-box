...
- **macOS / podman.** podman runs a Linux VM that shares your `$HOME`, so the `agent-box` dirs and your project must live **under `$HOME`**. Unlike bwrap's tmpfs home, the container's `$HOME` top level isn't itself writable — only the bound subdirs are (every path the agents use is one), so this is rarely noticeable.
